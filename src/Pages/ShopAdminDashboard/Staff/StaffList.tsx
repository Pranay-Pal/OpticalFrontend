...
export default function StaffList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const data = await ShopAdminAPI.staff.getAll({ page, limit: pageSize, status });
        ...
        <div className="flex flex-wrap gap-3 items-end mb-3">
          <div>
            ...
            <div className="flex gap-2 mt-1">
              <Button size="sm" variant={status==='ALL'?'default':'outline'} onClick={() => { setPage(1); setStatus('ALL'); }}>All</Button>
              <Button size="sm" variant={status==='ACTIVE'?'default':'outline'} onClick={() => { setPage(1); setStatus('ACTIVE'); }}>Active</Button>
              <Button size="sm" variant={status==='INACTIVE'?'default':'outline'} onClick={() => { setPage(1); setStatus('INACTIVE'); }}>Inactive</Button>
            </div>
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Page size</label>
            <Input className="w-24" type="number" min={5} max={100} value={pageSize} onChange={(e) => setPageSize(Math.min(100, Math.max(5, parseInt(e.target.value || '10', 10))))} />
          </div>
        </div>
                ...
                <td>
                  <div className="flex justify-end gap-2">
                    <Button asChild size="sm" variant="outline">
                      <Link to={`/shop-admin-dashboard/staff/${member.id}`}>View</Link>
                    </Button>
                    <Button asChild size="sm">
                      <Link to={`/shop-admin-dashboard/staff/${member.id}/activities`}>Activities</Link>
                    </Button>
                  </div>
                </td>
              </tr>
