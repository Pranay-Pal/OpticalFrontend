...
const CompanyCreateForm: React.FC<CompanyCreateFormProps> = ({ onCreated }) => {
	...
	useEffect(() => {
		const fetchCompanies = async () => {
			try { setLoadingCompanies(true); const data = await StaffAPI.inventory.getCompanies(); setCompanies(Array.isArray(data) ? data : (data?.items || [])); }
			catch { /* ignore */ }
	...
	const duplicate = useMemo(() => {
		...
		return companies.some(c => (c.name || '').toLowerCase() === name.trim().toLowerCase());
	}, [companies, name]);
	...
	const submit = async () => {
		...
		try {
			...
			setSuccess(res); onCreated?.(res); setName(''); setDescription('');
		} catch (e: any) { setError(e?.message || 'Creation failed'); }
	...
	return (
		<Card className="p-4 space-y-4">
			<div className="space-y-1">
				...
				<p className="text-xs text-muted-foreground">Register a manufacturer/vendor for product association.</p>
			</div>
			...
			<div className="space-y-3">
				<div>
					...
					<Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Acme Frames" />
					<div className="h-4">
						{!name.trim() && <span className="text-[10px] text-muted-foreground">Enter a name (min 2 chars)</span>}
						{name.trim() && duplicate && <span className="text-[10px] text-red-600">Name already exists</span>}
						{name.trim() && !duplicate && name.trim().length < 2 && <span className="text-[10px] text-red-600">Too short</span>}
					</div>
				</div>
				<div>
			...
			<div className="flex gap-2 flex-wrap">
				<Button onClick={submit} disabled={!valid || submitting}>{submitting ? 'Creating...' : 'Create Company'}</Button>
				<Button type="button" variant="ghost" size="sm" onClick={() => { setName(''); setDescription(''); setError(null); setSuccess(null); }} disabled={submitting}>Reset</Button>
			</div>
			<div className="pt-2">
				<p className="text-[11px] text-muted-foreground">Existing companies loaded{loadingCompanies ? '...' : `: ${companies.length}`}</p>
			</div>
		</Card>
	);
};
